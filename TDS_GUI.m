...
opts.Lower = [-5e3 -3e1 -1e1];
opts.Robust = 'Bisquare';
opts.Upper = [ 0.0  3e1  1e1];
...
for ii = 4:nLength-3
    ...
    % Fit for mass 2
    pPrs2max  =  max(pPrs2);
    [fit2, ~] =  fit(mass2-m2Pk,pPrs2/pPrs2max, ft, opts);
    pkMass2   = -fit2.p2/(2*fit2.p1);
    plotPres2 =  fit2(mass2-m2Pk)*pPrs2max;
    ...
    if peakPres3 < minPres || wghtPres3 > minWght
        ...
        plotPres3 =  fit3(mass3-m3Pk)*pPrs3max;
    ...
    if peakPres4 < minPres || wghtPres4 > minWght
        ...
        if fit4.p1 < 0
            pkMass4   = -fit4.p2/(2*fit4.p1);
            plotPres4 =  fit4(mass4-m4Pk)*pPrs4max;
...
jBgn = 1 + round(0.5*length(massN)/4);
jEnd = 1 + round(3.5*length(massN)/4);
...
for ii = 4:nLength-3
    ...
    % Fit for mass 2    
    pPrs2max  =  max(pPrs2);
    peakPres2 = mean(pPrs2);
    wghtPres2 =  std(pPrs2-peakPres2)/peakPres2;
    [fit2, ~] =  fit(mass2-m2Pk,pPrs2/pPrs2max,'poly2');
    if fit2.p1 < 0
        pkMass2   = -fit2.p2/(2*fit2.p1);
        plotPres2 =  fit2(mass2-m2Pk)*pPrs2max;
        peakPres2 =  fit2(pkMass2)*pPrs2max;
